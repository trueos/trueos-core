...
if [ -e "/usr/local/bin/startx" ] ; then
	dialog --title "TrueOS Installation Menu" --menu "Please select from the following options:" 20 55 15 xorg "Start graphical install " vesa "Start graphical install (VESA)" scfb "Start graphical install (SCFB)" intel "Start graphical install (Legacy Intel)" amd "Start graphical install (Radeon)" install "Start text install" utility "System Utilities" reboot "Reboot the system" 2>/tmp/answer
else
...
case $ANS in
    ...
    xorg) echo "Starting graphical Installer .. Please wait.."
          start_xorg ;;
