...
if [ -e "/usr/local/bin/startx" ] ; then
	dialog --title "TrueOS Installation Menu" --menu "Please select from the following options:" 20 55 15 xorg "Start graphical install (Auto-Detect Video) " failsafe "Start graphical install (Failsafe mode)" vesa "Start graphical install (VESA)" scfb "Start graphical install (SCFB)" install "Start text install" utility "System Utilities" reboot "Reboot the system" 2>/tmp/answer
else
